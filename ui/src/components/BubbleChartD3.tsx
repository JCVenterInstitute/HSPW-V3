...
export const BubblePlot = ({ width, height, data }: BubblePlotProps) => {
  ...
  const yScale = useMemo(() => {
    const [min, max] = d3.extent(data.map((d) => d.frequency)) as [
      number,
      number
    ];
  ...
  const xScale = useMemo(() => {
    const [min, max] = d3.extent(data.map((d) => d.number_of_member)) as [
      number,
      number
    ];
    ...
    .map((d, i) => {
      return (
        <circle
          key={i}
          r={sizeScale(d.pop)}
          cx={xScale(d.number_of_member)}
          cy={yScale(d.frequency)}
          opacity={1}
          stroke={"#e85252"}
          fill={"#e85252"}
          fillOpacity={0.4}
