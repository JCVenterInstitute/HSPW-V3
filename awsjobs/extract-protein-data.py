...
def move_file_to(bucket_name, folder_name, file_name):
    try:
        source_key = f'uniprot_source_files/{file_name}'
        destination_key = f'uniprot_source_files/{folder_name}/{file_name}'
        s3_client.copy_object(Bucket=bucket_name, CopySource={'Bucket': bucket_name, 'Key': source_key}, Key=destination_key)
        s3_client.delete_object(Bucket=bucket_name, Key=source_key)
        print(f"Moved uniprot protein source file {file_name} to {bucket_name}/uniprot_source_files/{folder_name}")
    except ClientError as e:
        logger.exception(f"An error occurred while moving the file {file_name}: {e}")

# Process protein files
def process_protein_file(protein_files_bucket, uniprot_proteins_bucket, protein_id):
    uniprot_file_name = f"{protein_id}.json"
    uniprot_data = read_uniprot_data(protein_files_bucket, f'uniprot_source_files/{uniprot_file_name}')
    ...
    if uniprot_data:
        ...
        try:
            protein_extract = create_protein_extract(uniprot_data)

            if protein_extract:
                print(f"{protein_extract_name} created successfully!")

                response_put = s3_client.put_object(
                    Bucket=uniprot_proteins_bucket,
                    Key=protein_extract_name,
                    Body=json.dumps(protein_extract)
                )
                if response_put:
                    print(f"Saved protein extract to S3 bucket {uniprot_proteins_bucket}")
                    move_file_to(protein_files_bucket, 'completed', uniprot_file_name)
                    return True
        except Exception as e:
            logger.exception(f"An error occurred while processing file {uniprot_file_name}: {e}")
            move_file_to(protein_files_bucket, 'failed', uniprot_file_name)
...
def main():
    ...
    for protein_id in protein_ids:
        try:
            protein_extract = process_protein_file(protein_files_bucket, uniprot_proteins_bucket, protein_id)
            if protein_extract:
                completed +=1
