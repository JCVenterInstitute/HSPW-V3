...
def main():
    ...
    mz_tab, protein_df, peptide_df = parse_mztab(args.mztab_file)
    ...
    save_json(
        ...
        os.path.join(
            args.output_dir,
            os.path.basename(args.mztab_file).split(".")[0] + "_study.json",
        ),
    )
    save_json(
        study_protein,
        os.path.join(
            args.output_dir,
            os.path.basename(args.mztab_file).split(".")[0] + "_study_protein.json",
        ),
    )
    save_json(
        study_peptide,
        os.path.join(
            args.output_dir,
            os.path.basename(args.mztab_file).split(".")[0] + "_study_peptide.json",
        ),
    )
