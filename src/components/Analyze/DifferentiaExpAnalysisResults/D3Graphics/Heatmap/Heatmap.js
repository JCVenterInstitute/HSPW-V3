...
//       console.log(">>>>>columns[3]:", columns[18]);
...
//         .range([height, 0])
//         .padding(0.05);
//       console.log(">>>>>labels[134]:", labels[218]);
...
const HeatmapComponent = ({ jobId, fileName, numbVolcanoSamples, tab }) => {
  const svgRef = useRef();
  const [data, setData] = useState([]);
  const [cleanedrowOrder, setCleanedrowOrder] = useState([]);
  const [cleanedcolOrder, setCleanedcolOrder] = useState([]);
  ...
  const cleanTsvData = (data) => {
    if (!Array.isArray(data) || data.length === 0) return [];
    return data.map((d) => +d.x).filter((value) => value);
  };
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      const xScale = d3
        .scaleBand()
        .domain(cleanedcolOrder.map((index) => columns[index]))
        .range([0, width])
        .padding(0.05);
      ...
      const yScale = d3
        .scaleBand()
        .domain(reversedcleanedrowOrder.map((index) => labels[index]))
        .range([height, 0])
        .padding(0.05);
      ...
      // Create a separate group for the heatmap
      const heatmapGroup = plot.append("g").attr("class", "heatmap-group");
      ...
      const zoom = d3
        ...
        .on("zoom", (event) => {
          ...
          heatmapGroup.attr("transform", transform); // Apply transformation only to the heatmap
        });
      ...
      // Append axes and labels to the plot
      plot
        .append("g")
        .attr("class", "x-axis")
        .attr("transform", `translate(0, ${height})`)
  ...
  }, [data, cleanedrowOrder, cleanedcolOrder]);
