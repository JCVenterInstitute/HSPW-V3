...
//       // Filter out proteins that are blank/empty for all the Description values in the first 25 rows of data1
...
const HeatmapComponent = ({ jobId, fileName1, fileName2, selectedSection }) => {
  ...
  return (
    <div className="heatmap-container">
      <div className="tabs">
        <button onClick={() => handleTabClick("All Samples")}>
          All Samples
        </button>
        <button onClick={() => handleTabClick("Compressed graph")}>
          Compressed graph
        </button>
      </div>
      <svg
