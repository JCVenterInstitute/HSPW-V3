...
def main(event):
    ...
    try:
        ...
        result = subprocess.run("Rscript ../metab4script.R", shell=True)
        if result.returncode != 0:
            print(f"> R script failed: {result.stderr.decode('utf-8')}")
            return {
                'statusCode': 500,
                'body': json.dumps({'message': 'R script execution failed'}),
        ...
        r_command = ["Rscript", "script.R", log_normalized, fold_threshold, p_val, p_raw, stat_test, heat_map_number]
        print("> Running R script with command:", r_command)
        result = subprocess.run(r_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
