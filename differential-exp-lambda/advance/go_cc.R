...
set.seed(1234)
egoccup <- enrichGO(gene = pup$rn, OrgDb = org.Hs.eg.db, keyType = 'UNIPROT', ont = "CC", pAdjustMethod = "BH", pvalueCutoff  = pvalue, qvalueCutoff  = qvalue)
write.table(as.data.frame(egoccup),"egoccup.tsv",sep="\t",row.names = TRUE)
set.seed(1234)
egoccdown <- enrichGO(gene = pdown$rn, OrgDb = org.Hs.eg.db, keyType = 'UNIPROT', ont = "CC", pAdjustMethod = "BH", pvalueCutoff  = pvalue, qvalueCutoff  = qvalue)
write.table(as.data.frame(egoccdown),"egoccdown.tsv",sep="\t",row.names = TRUE)
...
ridgecc <- ridgeplot(gsecc,25,core_enrichment=FALSE)
