...
def download_file_from_s3(bucket_name, file_name, download_dest):
    ...
    print(f"> Download destination: '{download_dest}'")

    try:
        # Ensure destination directory exists
        dest_dir = os.path.dirname(download_dest)
...
def main(event):
    ...
    try:
        ...
        file_name = f"{dirName}/all_data.tsv"
        ...
        upload_files_to_s3(s3_bucket_name, directory_name, dirName)
...
def handler(event, context):
    ...
    if event.get("httpMethod") == "GET":
        ...
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "Advance Analysis API Pinged"}),
        }
