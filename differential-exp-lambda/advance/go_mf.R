...
set.seed(1234)
egomfup <- enrichGO(gene = pup$rn, OrgDb = org.Hs.eg.db, keyType = 'UNIPROT', ont = "MF", pAdjustMethod = "BH", pvalueCutoff  = pvalue, qvalueCutoff  = qvalue)
write.table(as.data.frame(egomfup),"egomfup.tsv",sep="\t",row.names = TRUE)
set.seed(1234)
egomfdown <- enrichGO(gene = pdown$rn, OrgDb = org.Hs.eg.db, keyType = 'UNIPROT', ont = "MF", pAdjustMethod = "BH", pvalueCutoff  = pvalue, qvalueCutoff  = qvalue)
